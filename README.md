...
A 股日频横截面研究框架。第一阶段只产出研究证据，不连接实盘订单系统。
M10 额外提供一个 `SHADOW_ONLY` 的合成信号运行控制面；它不包含任何
live-order 接口。
...
- 轨道：`STRICT`
- 当前阶段：`M7-ISOLATED-INCREMENTS-PREFLIGHT`
- 真实数据研究状态：`PIT_QUALIFIED_DEVELOPMENT_ONLY`
- 允许：在不改变数据、切分与 score 接口的前提下，分别冻结并检验 CCC
  和小型市场状态 Gate
- 禁止：CCC 与 Gate 未分别有效前的组合实验、2025+ 最终 OOS 访问、
  未登记调参、Alpha/可投资性结论和实盘连接
...
- K16 与 K32 MSE 各完成七个冻结滚动折，14 个检查点均精确回放；
- K16 `wf_2018` 独立重训与原预测逐值完全一致；
- 1,898,028 行预测通过独立哈希、模式、唯一键、有限值和日期边界检查；
- 两套 Qlib Recorder 产物由独立验证器重新下载并逐字节验签；
- 参数量分别为 29,521 和 30,561，复杂度保持 O(NK)；
- 累计试验消耗为 6 次候选评估、44 次模型拟合。

M6 没有根据结果选择 K，也没有比较基准、组合、成本或容量。下一步只
允许把 CCC 与小型市场状态 Gate 分别冻结、分别检验；两者未各自通过前
不得组合。
