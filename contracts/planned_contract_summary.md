...
14 issue(s): 0 hard stop, 0 redesign, 13 evidence gap, 1 claim limit.

- `NEEDS_EVIDENCE` `NEEDS_EVIDENCE` `data.sources[0].availability_evidence_hash` — must be a non-empty string before freeze
- `NEEDS_EVIDENCE` `NEEDS_EVIDENCE` `data.sources[0].availability_evidence_uri` — must be a non-empty string before freeze
- `NEEDS_EVIDENCE` `NEEDS_EVIDENCE` `data.sources[0].availability_evidence_version` — must be a non-empty string before freeze
- `NEEDS_EVIDENCE` `NEEDS_EVIDENCE` `data.sources[0].availability_timestamp_field` — must be a non-empty string before freeze
...
- `NEEDS_EVIDENCE` `NEEDS_EVIDENCE` `data.sources[1].availability_evidence_hash` — must be a non-empty string before freeze
- `NEEDS_EVIDENCE` `NEEDS_EVIDENCE` `data.sources[1].availability_evidence_uri` — must be a non-empty string before freeze
- … 6 additional issue(s); use the validator for the full list.

## Research paths

### A — Evidence or repair first (recommended)

Resolve `data.sources[0].availability_evidence_hash` so the decision can change. Produces: a directly reviewable definition or artifact. Does not prove: the remaining contract or alpha.
