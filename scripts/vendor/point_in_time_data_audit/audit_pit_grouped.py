...
def streamed_list_sha256(items: Any) -> str:
...
class GroupedFallbackRequired(ValueError):
    """The input is valid long-form data but cannot be safely collapsed."""
...
class VirtualLongRow(dict[str, str]):
    """A dict-compatible view over one feature cell in a grouped sample."""

    __slots__ = ("_rows", "_sample_index", "_cell_index")

    def __init__(
        self,
        rows: GroupedLongRows,
        sample_index: int,
        cell_index: int,
    ) -> None:
        super().__init__()
    ...
    def get(self, key: str, default: Any = None) -> Any:
        special = self._rows.special_index.get(key)
        if special == 0:
            return self._rows.row_ids[self._sample_index][self._cell_index]
        if special == 1:
            return self._rows.feature_names_by_sample[self._sample_index][
                self._cell_index
            ]
        if special == 2:
            return self._rows.feature_values[self._sample_index][self._cell_index]
        base_index = self._rows.base_index.get(key)
        if base_index is None:
            return default
        return self._rows.base_values[self._sample_index][base_index]
...
class GroupedLongRows:
    """Memory-bounded exact representation of rectangular long-form evidence."""

    def __init__(
        self,
        headers: list[str],
        base_headers: list[str],
        base_values: list[tuple[str, ...]],
        row_ids: list[tuple[str, ...]],
        feature_names_by_sample: list[tuple[str, ...]],
        feature_values: list[tuple[str, ...]],
        row_id_convention_violations: list[str],
        row_id_convention_violation_count: int,
        cell_value_violations: list[dict[str, Any]],
        cell_value_violation_count: int,
    ) -> None:
        self.headers = headers
        self.base_headers = tuple(base_headers)
        self.base_index = {
            name: position for position, name in enumerate(self.base_headers)
        }
        self.special_index = {
            "row_id": 0,
            "feature_name": 1,
        ...
        self.feature_names_by_sample = feature_names_by_sample
        self.feature_values = feature_values
        self.row_id_convention_violations = row_id_convention_violations
        self.row_id_convention_violation_count = (
            row_id_convention_violation_count
        )
        self.cell_value_violations = cell_value_violations
        self.cell_value_violation_count = cell_value_violation_count
        self._length = sum(len(items) for items in feature_names_by_sample)

    def __len__(self) -> int:
        return self._length

    def __iter__(self) -> Any:
        for sample_index, names in enumerate(self.feature_names_by_sample):
            for cell_index in range(len(names)):
                yield VirtualLongRow(self, sample_index, cell_index)

    def iter_samples(self) -> Any:
        for sample_index, names in enumerate(self.feature_names_by_sample):
            if names:
                yield VirtualLongRow(self, sample_index, 0)

    def sample_key(self, sample_index: int) -> tuple[str, str]:
        security_id = self.base_values[sample_index][
            self.base_index["security_id"]
        ]
        prediction_time = self.base_values[sample_index][
            self.base_index["prediction_time"]
        ]
        return security_id, normalized_time_text(prediction_time)

    def observed_feature_names(self) -> list[str]:
        return sorted(
            {
                name
                for names in self.feature_names_by_sample
                for name in names
                if name
            }
        )
...
class GroupedLongTimes:
    """Repeat one parsed time bundle for every feature cell without copying."""

    def __init__(
        self,
        rows: GroupedLongRows,
        sample_times: list[dict[str, datetime | None]],
    ) -> None:
        self.rows = rows
    ...
    def __iter__(self) -> Any:
        for times, names in zip(
            self.sample_times, self.rows.feature_names_by_sample
        ):
            for _ in names:
                yield times

    def iter_samples(self) -> Any:
        return iter(self.sample_times)


def strict_csv_grouped_rows(
    path: Path, artifact: str
) -> tuple[GroupedLongRows, list[str]]:
    """Parse exact long rows while storing repeated sample metadata only once."""

    cell_fields = ("row_id", "feature_name", "feature_value")
    with path.open(encoding="utf-8", newline="") as handle:
        reader = csv.reader(handle)
        try:
            headers = next(reader)
        ...
        missing = [field for field in cell_fields if field not in headers]
        if missing:
            raise CsvStructureError(
                f"{artifact}.header",
                f"grouped rectangular mode requires fields: {missing}",
            )
        index = {name: position for position, name in enumerate(headers)}
        base_headers = [
            name for name in headers if name not in set(cell_fields)
        ]
        base_positions = [index[name] for name in base_headers]
        key_positions = (index["security_id"], index["prediction_time"])
        base_values: list[tuple[str, ...]] = []
        row_ids: list[tuple[str, ...]] = []
        feature_names_by_sample: list[tuple[str, ...]] = []
        feature_values: list[tuple[str, ...]] = []
        ...
        cell_value_violations: list[dict[str, Any]] = []
        cell_value_violation_count = 0
        completed: set[tuple[str, str]] = set()
        current_key: tuple[str, str] | None = None
        current_base: tuple[str, ...] | None = None
        current_row_ids: list[str] = []
        current_names: list[str] = []
        current_values: list[str] = []

        def finish_sample() -> None:
            if current_key is None or current_base is None:
                return
            completed.add(current_key)
            base_values.append(current_base)
            row_ids.append(tuple(current_row_ids))
            feature_names_by_sample.append(tuple(current_names))
            feature_values.append(tuple(current_values))
        ...
        for row_number, values in enumerate(reader, start=2):
            ...
            key = (values[key_positions[0]], values[key_positions[1]])
            base = tuple(values[position] for position in base_positions)
            if key != current_key:
                finish_sample()
                if key in completed:
                    raise GroupedFallbackRequired(
                        "security/prediction sample is not contiguous"
                    )
                current_key = key
                current_base = base
                current_row_ids = []
                ...
                raise GroupedFallbackRequired(
                    "feature rows in one sample have inconsistent metadata"
                )
            row_id = values[index["row_id"]]
            feature_name = sys.intern(values[index["feature_name"]])
            feature_value = values[index["feature_value"]]
            cell_issue = None
            if not row_id or not feature_name or not feature_value:
                cell_issue = "required cell value is missing"
            else:
                try:
                    if not math.isfinite(float(feature_value)):
                        raise ValueError("non-finite numeric")
                except (TypeError, ValueError) as error:
                    cell_issue = f"feature_value type validation failed: {error}"
            if cell_issue is not None:
                cell_value_violation_count += 1
                if len(cell_value_violations) < 25:
                    cell_value_violations.append(
                        {
                            "locator": f"data#row_id={row_id or '<missing>'}",
                            "reason": cell_issue,
                        }
                    )
            prediction_date = key[1][:10].replace("-", "")
            expected_row_id = f"{key[0]}:{prediction_date}:{feature_name}"
            if row_id != expected_row_id:
                convention_violation_count += 1
                if len(convention_violations) < 25:
                    convention_violations.append(row_id)
            current_row_ids.append(row_id)
            current_names.append(feature_name)
            current_values.append(feature_value)
        finish_sample()
    return (
        GroupedLongRows(
            headers,
            ...
            base_values,
            row_ids,
            feature_names_by_sample,
            feature_values,
            convention_violations,
            convention_violation_count,
            cell_value_violations,
            cell_value_violation_count,
        ),
        headers,
    )
...
class Audit:
    ...
    def audited_feature_names(self) -> list[str]:
        if isinstance(self.rows, GroupedLongRows):
            observed = self.rows.observed_feature_names()
        else:
            observed = [
    ...
    def sample_rows(self) -> Any:
        if isinstance(self.rows, GroupedLongRows):
            return self.rows.iter_samples()
        return iter(self.rows)

    def sample_pairs(self) -> Any:
        if isinstance(self.rows, GroupedLongRows) and isinstance(
            self.times, GroupedLongTimes
        ):
            return zip(self.rows.iter_samples(), self.times.iter_samples())
        return zip(self.rows, self.times)

    def weighted_counter(self, field: str) -> Counter[Any]:
        if isinstance(self.rows, GroupedLongRows):
            result: Counter[Any] = Counter()
            for sample_index, row in enumerate(
                self.rows.iter_samples()
            ):
                result[row.get(field)] += len(
                    self.rows.feature_names_by_sample[sample_index]
                )
            return result
        return Counter(row.get(field) for row in self.rows)

    def unique_sample_values(self, field: str) -> set[Any]:
        return {row.get(field) for row in self.sample_rows()}
            ...
            for name in sorted(artifact_names):
                ...
                try:
                    ...
                    if name in {"data", "calendar"}:
                        if (
                            name == "data"
                            and scope == "FULL_TRAINING_INPUT"
                        ):
                            try:
                                rows, headers = strict_csv_grouped_rows(
                                    path, name
                                )
                            except GroupedFallbackRequired:
    ...
    def check_i002_contract(self) -> None:
        ...
        if source is None:
            ...
            used = set(self.audited_feature_names())
        ...
        self.add(
            ...
            metrics={"used_features": len(self.audited_feature_names())},
            repair=(
    ...
    def check_t001_time_roles(self) -> None:
        ...
        self.times = (
            GroupedLongTimes(self.rows, parsed_times)
            if isinstance(self.rows, GroupedLongRows)
            else parsed_times
        )
        self.add(
    ...
    def check_t002_grouped(self) -> None:
        rows = self.rows
        if not isinstance(rows, GroupedLongRows):
            raise TypeError("grouped T002 requires GroupedLongRows")
        failures: list[Any] = []
        gaps: list[Any] = []
        op = self.docs.get("time_semantics", {}).get(
            "availability_comparison"
        )
        claim = self.request.get("claim")
        features = self.docs.get("dictionary", {}).get("features", {})
        feature_policies = {
            name: features.get(name, {}).get("event_time_policy")
            for name in rows.observed_feature_names()
        }
        ...
        observed_policy_present = any(
            policy == "OBSERVED_BY_PREDICTION"
            for policy in feature_policies.values()
        )
        market_count = system_count = causal_count = 0
        sample_times = self.times
        if not isinstance(sample_times, GroupedLongTimes):
            raise TypeError("grouped T002 requires GroupedLongTimes")
        for sample_index, (row, times) in enumerate(
            zip(rows.iter_samples(), sample_times.iter_samples())
        ):
            cell_count = len(
                rows.feature_names_by_sample[sample_index]
            )
            prediction = times.get("prediction_time")
            market_fields = ["published_time", "vendor_available_time"]
            system_fields = market_fields + [
                "ingested_time",
                "parse_ready_time",
            ]
            needed = (
            ...
            if prediction is None or any(
                times.get(key) is None for key in needed
            ):
                gaps.append(
                    self.issue(
                        locator(row),
                        "availability predicate is incomplete",
                    )
                )
                continue
            causal_fields = [
            ...
            if any(times.get(key) is None for key in causal_fields):
                ...
                inversions = [
                    ...
                    for left, right in zip(
                        causal_fields, causal_fields[1:]
                    )
                    if times[left] > times[right]
                ]
                if (
                    observed_policy_present
                    and times.get("event_time") is not None
                    and times["event_time"] > times["published_time"]
                ):
                    inversions.insert(0, ("event_time", "published_time"))
                if inversions:
                    ...
                    causal_count += cell_count
            market_ok = all(
                before(times[key], prediction, op)
            ...
            if claim == "SYSTEM_REPLAYABLE":
                ...
                if not batch_id or not object_hash:
                    ...
                    system_receipt_ok = False
            system_ok = system_time_ok and system_receipt_ok
            market_count += int(market_ok) * cell_count
            system_count += int(system_ok) * cell_count
            selected_time_ok = (
                system_time_ok
                if claim == "SYSTEM_REPLAYABLE"
                else market_ok
            ...
            event = times.get("event_time")
            if observed_policy_present and (
                event is None or not before(event, prediction, op)
            ):
                failures.append(
                    self.issue(
                        locator(row),
                        "observed event occurs after prediction",
                    )
                )
            tradable = times.get("tradable_time")
            if tradable is None:
                gaps.append(
                    self.issue(
                        locator(row), "tradable time is unavailable"
                    )
                )
            elif prediction > tradable:
                failures.append(
                    self.issue(
                        locator(row),
                        "tradable time precedes the prediction decision",
                    )
                )
        self.add(
            "T002",
            ...
            self.evidence(
                {
                    ...
                    "memory_execution_mode": "GROUPED_RECTANGULAR_EXACT",
                }
            ),
            metrics={
                "market_reconstructible_rows": market_count,
    ...
    def check_t003_revision(self) -> None:
        failures, gaps = [], []
        op = self.docs.get("time_semantics", {}).get("availability_comparison")
        for row, times in self.sample_pairs():
            known, prediction = (
        ...
        self.add(
            ...
            metrics={
                "revision_ids": len(
                    {r.get("revision_id") for r in self.sample_rows()}
                )
            },
        ...
        self.add(
            ...
            metrics={
                ...
                "security_status_counts": dict(
                    self.weighted_counter("security_status")
                ),
            },
            repair=(
    ...
    def check_m001_identity(self) -> None:
        ...
        self.add(
            ...
            metrics={
                "security_ids": len(
                    self.unique_sample_values("security_id")
                )
            },
            repair=(
    ...
    def check_a001_grouped(self) -> None:
        rows = self.rows
        if not isinstance(rows, GroupedLongRows):
            raise TypeError("grouped A001 requires GroupedLongRows")
        failures: list[Any] = []
        gaps: list[Any] = []
        features = self.docs.get("dictionary", {}).get("features", {})
        policies: dict[str, dict[str, Any]] = {}
        for name in rows.observed_feature_names():
            adjustment_policy = features.get(name, {}).get(
                "adjustment_policy"
            )
            if not isinstance(adjustment_policy, dict):
                gaps.append(
                    self.issue(
                        f"dictionary.features.{name}.adjustment_policy",
                        "adjustment basis and convention are missing",
                    )
                )
                adjustment_policy = {}
                ...
                for key in (
                    ...
                    if not adjustment_policy.get(key):
                        gaps.append(
                            self.issue(
                                f"dictionary.features.{name}.adjustment_policy.{key}",
                                "adjustment evidence is missing",
                            )
                        )
            policies[name] = adjustment_policy

        mode_counts: Counter[str] = Counter()
        for sample_index, row in enumerate(rows.iter_samples()):
            mode = row.get("adjustment_mode")
            names = rows.feature_names_by_sample[sample_index]
            mode_counts[mode] += len(names)
            incompatible = [
                name
                for name in names
                if policies.get(name, {}).get("allowed_modes")
                and mode
                not in policies[name]["allowed_modes"]
            ]
            if incompatible:
                failures.append(
                    self.issue(
                        ...
                        "consumed adjustment mode conflicts with feature semantics",
                        mode=mode,
                        feature_examples=sorted(set(incompatible))[:5],
                    )
                )
            if mode == "RAW":
                continue
        ...
        self.add(
            ...
            metrics={"adjustment_modes": dict(mode_counts)},
            repair=(
                "Use raw values. Supporting adjusted values requires a new source-bound "
                "proof artifact that recomputes value, factor, ex-date, and base-date."
            ),
    ...
    def check_q001_grouped(self) -> None:
        rows = self.rows
        if not isinstance(rows, GroupedLongRows):
            raise TypeError("grouped Q001 requires GroupedLongRows")
        schema = self.docs.get("schema", {})
        failures: list[Any] = []
        gaps: list[Any] = []
        primary = schema.get("primary_key", [])
        grain = schema.get("grain", [])
        expected_primary = ["row_id"]
        expected_grain = [
            "security_id",
            "prediction_time",
            "feature_name",
        ...
        if primary != expected_primary:
            failures.append(
                self.issue(
                    "schema.primary_key",
                    "grouped rectangular executor requires the exact row_id primary key",
        ...
        if grain != expected_grain:
            failures.append(
                self.issue(
                    "schema.grain",
                    "grouped rectangular executor requires the exact matrix grain",
                    expected=expected_grain,
                    actual=grain,
                )
            )
        row_id_duplicate_count = 0
        if rows.row_id_convention_violation_count:
            if len(rows) <= 5_000_000:
                seen_row_ids: set[str] = set()
                for sample_row_ids in rows.row_ids:
                    for row_id in sample_row_ids:
                        if row_id in seen_row_ids:
                            row_id_duplicate_count += 1
                        seen_row_ids.add(row_id)
                if row_id_duplicate_count:
                    failures.append(
                        self.issue(
                            "data.row_id",
                            "duplicate primary row_id",
                            duplicate_count=row_id_duplicate_count,
                        )
                    )
            else:
                gaps.append(
                    self.issue(
                        "data.row_id",
                        "non-canonical row IDs require an external-sort uniqueness proof in grouped mode",
                        examples=rows.row_id_convention_violations[:5],
                    )
                )

        observed_by_key: dict[tuple[str, str], tuple[str, ...]] = {}
        duplicate_sample_keys: list[tuple[str, str]] = []
        duplicate_feature_cells = 0
        for sample_index, names in enumerate(
            rows.feature_names_by_sample
        ):
            try:
                key = rows.sample_key(sample_index)
            except ValueError as error:
                failures.append(
                    self.issue(
                        locator(
                            VirtualLongRow(rows, sample_index, 0)
                        ),
                        f"invalid normalized matrix key: {error}",
                    )
                )
                continue
            if key in observed_by_key:
                if len(duplicate_sample_keys) < 5:
                    duplicate_sample_keys.append(key)
            else:
                observed_by_key[key] = names
            duplicate_feature_cells += len(names) - len(set(names))
        if duplicate_sample_keys:
            failures.append(
                self.issue(
                    "data",
                    "duplicate normalized security/prediction samples",
                    examples=[list(item) for item in duplicate_sample_keys],
                )
            )
        if duplicate_feature_cells:
            failures.append(
                self.issue(
                    "data",
                    "duplicate feature cells occur within a sample",
                    duplicate_count=duplicate_feature_cells,
                )
            )
        ...
        expected_count = 0
        observed_unique_count = sum(
            len(set(names)) for names in observed_by_key.values()
        )
        if not matrix_manifest:
            ...
            if not valid_features:
                failures.append(
                    self.issue(
                        "matrix_manifest.feature_axis.features",
            ...
            if (
                ...
                or feature_axis.get("feature_set_sha256")
                != sha_value(features)
            ):
                failures.append(
                    self.issue(
                        "matrix_manifest.feature_axis",
                        "feature count/digest mismatch",
                    )
                )
            ...
            for index, item in enumerate(
                path_get(
                    self.docs.get("population_manifest", {}), "keys", []
                )
            ):
                try:
                    population_keys.add(
                    ...
                    failures.append(
                        self.issue(
                            f"population_manifest.keys[{index}]",
                            "invalid matrix-axis key",
                        )
                    )
            expected_count = len(population_keys) * len(features)
            expected_digest = streamed_list_sha256(
                [security_id, prediction_time, feature]
                for security_id, prediction_time in sorted(population_keys)
                for feature in features
            )
            declared_count = matrix_manifest.get("expected_cell_count")
            if (
                ...
                failures.append(
                    self.issue(
                        "matrix_manifest",
                        "rectangular cell count/digest mismatch",
                    )
                )

            observed_digest = streamed_list_sha256(
                [security_id, prediction_time, feature]
                for (
                    security_id,
                    prediction_time,
                ) in sorted(observed_by_key)
                for feature in sorted(
                    set(observed_by_key[(security_id, prediction_time)])
                )
            )
            missing_samples = population_keys - set(observed_by_key)
            extra_samples = set(observed_by_key) - population_keys
            feature_mismatch_keys = [
                key
                for key in sorted(population_keys & set(observed_by_key))
                if set(observed_by_key[key]) != set(features)
                or len(observed_by_key[key])
                != len(set(observed_by_key[key]))
            ]
            if (
                missing_samples
                or extra_samples
                or feature_mismatch_keys
                or observed_unique_count != expected_count
                or observed_digest != expected_digest
            ):
                missing_examples: list[list[str]] = []
                extra_examples: list[list[str]] = []
                for key in sorted(missing_samples)[:5]:
                    if features:
                        missing_examples.append([*key, features[0]])
                for key in sorted(extra_samples)[:5]:
                    names = observed_by_key.get(key, ())
                    if names:
                        extra_examples.append([*key, names[0]])
                for key in feature_mismatch_keys[:5]:
                    missing_features = sorted(
                        set(features) - set(observed_by_key[key])
                    )
                    extra_features = sorted(
                        set(observed_by_key[key]) - set(features)
                    )
                    if missing_features:
                        missing_examples.append([*key, missing_features[0]])
                    if extra_features:
                        extra_examples.append([*key, extra_features[0]])
                failures.append(
                    self.issue(
                        "matrix_manifest.expected_cell_key_set_sha256",
                        "materialized matrix cells differ from the frozen exact set",
                        missing_count=max(
                            0, expected_count - observed_unique_count
                        ),
                        extra_count=max(
                            0, observed_unique_count - expected_count
                        ),
                        missing_examples=missing_examples[:5],
                        extra_examples=extra_examples[:5],
                    )
                )

        unique_cells = observed_unique_count
        self.add(
            "Q001",
            failures,
            gaps,
            ...
            metrics={
                "unique_primary_keys": unique_cells,
                "unique_grain_keys": unique_cells,
                "expected_matrix_cells": expected_count,
                "observed_matrix_cells": unique_cells,
            },
            repair=(
    ...
    def check_q002_grouped(self) -> None:
        rows = self.rows
        if not isinstance(rows, GroupedLongRows):
            raise TypeError("grouped Q002 requires GroupedLongRows")
        failures: list[Any] = list(rows.cell_value_violations)
        gaps: list[Any] = []
        columns = self.docs.get("schema", {}).get("columns", {})
        parsers = {
            "string": lambda value: value,
            "float": lambda value: (
                float(value)
        ...
        expected_special_types = {
            "row_id": "string",
            "feature_name": "string",
            "feature_value": "float",
        }
        for field, expected_type in expected_special_types.items():
            spec = columns.get(field, {})
            if spec.get("type") != expected_type:
                failures.append(
                    self.issue(
                        f"schema.columns.{field}.type",
                        "grouped executor special-field type conflicts with the schema",
                        expected=expected_type,
                        actual=spec.get("type"),
                    )
                )
        for row in rows.iter_samples():
            for field, spec in columns.items():
                if field in expected_special_types:
                    continue
                raw = row.get(field, "")
                if raw == "":
                    if spec.get("nullable") is not True:
        ...
        if rows.cell_value_violation_count > len(
            rows.cell_value_violations
        ):
            failures.extend(
                self.issue(
                    "data.feature_value",
                    "additional malformed feature cells were detected",
                    additional_count=(
                        rows.cell_value_violation_count
                        - len(rows.cell_value_violations)
                    ),
                )
                for _ in range(1)
            )
        self.add(
            ...
            metrics={
                "validated_cells": len(rows) * len(columns),
                "malformed_feature_cells": rows.cell_value_violation_count,
            },
            repair=(
                "Repair only malformed or missing source values; preserve the frozen schema."
            ),
        )
    ...
    def check_q003_domains(self) -> None:
        failures, gaps = [], []
        features = self.docs.get("dictionary", {}).get("features", {})
        observed_names = self.audited_feature_names()
        ...
        value_rows = (
            self.rows
            if not isinstance(self.rows, GroupedLongRows)
            or bounded_names
            else ()
        )
    ...
    def manifest(self) -> dict[str, Any]:
        ...
        if isinstance(self.rows, GroupedLongRows):
            observed_security_time_keys = len(self.rows.base_values)
            observed_matrix_cells = sum(
                len(set(names))
                for names in self.rows.feature_names_by_sample
            )
            memory_execution_mode = "GROUPED_RECTANGULAR_EXACT"
        else:
            ...
            memory_execution_mode = "MATERIALIZED_LONG_ROWS"
        core = {
            ...
            "runtime_execution": {
                "mode": memory_execution_mode,
                "executor_sha256": sha_file(Path(__file__).resolve()),
                "semantic_rule_set": "pit_audit_manifest_v1_17_checks",
            },
            ...
            "coverage_matrix": {
                ...
                "observed_security_time_keys": observed_security_time_keys,
                "expected_matrix_cells": path_get(
                    self.docs.get("matrix_manifest", {}),
                    "expected_cell_count",
                ),
                "observed_matrix_cells": observed_matrix_cells,
